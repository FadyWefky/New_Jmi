...
{
  // Mail::send('mails.newaccount2',['email' => 'bishoyshenouda2019@gmail.com','fullname' => 'bbbb ttttt'], function($message){
  //         $message->to('shokeryfarg@gmail.com');
...
public function test()
{
    ...
    $data['name']='Ibrahim';
    $data['details']="
    JMIBrokers has been serving and empowering traders since 2009 (that’s 13 years+!) and been with them through thick and thin! By now we substantially know what the hassles are and what each trader wishes to find.
Consequently, throughout the years we’ve been scoping our work and offers to cater accordingly.
Each trader to his preferred strategy, risk management, level of expertise, and ease of execution.
First things first: Compliance! JMIBrokers is licensed from the Common Wealth (VFSC registration 15010) and fully owned by the Swiss JMIBrokers LTD (registered office in Zug with registration # CHE 334-229-499). Over and above that, all traders’ deposits are insured up to $500,000!  (https://www.jmibrokers.com/assets/Downloads/JMI%20BROKERS%20LIMITED%20PI%20COVER.pdf)
With JMIBrokers you can trade with diverse markets of Forex Currency pairs, Futures (OTC), Precious Metals, Indices, Equities/stocks, and Energies all on a single platform through 3 types of trading accounts cut-out for your trading character and choice: FIXED SPREAD Account, VARIABLE SPREAD Account, and SCALPING Account with a minimum deposit of $100 and leverage up to 1:500.
But it gets much better than this! We are presenting the best opportunities to capitalize on with our new favored trading options: "
. "<img src='https://www.jmibrokers.com/mails/mail.jpeg'> " ."<br>" .
"<img src='https://www.jmibrokers.com/mails/mail2.jpeg'>" . "<br>" . "
<img src='https://www.jmibrokers.com/mails/mail3.jpeg'>" .
"

The ease of deposit and withdrawal methods with JMIBrokers are impeccable and widespread. Aside from SWIFT through corporate bank accounts in USA, London, Singapore, SEPA and Dubai, JMIBrokers also works through Western Union, Money Gram, Coinbase, Paypeer, Perfect Money, Advcash, Epay and crypo currencies.
Our experts are ready to consult with you on risk management and best choice of trading accounts and are today  granting the first 150 replies a free 45minute consultation call! Connect now! <(hyperlink to a reply form)
JMIBrokers also allows you to trade risk free with virtual cash!
Put your trading skills to the test with our Demo Account; submit trades in a virtual environment before you start risking your own money. Practice trading strategies so that when you're ready to enter the real market, you've had the practice you need. Try our demo today >> (hyperlink to demo account)
Let’s launch the New Year together with prosperity at heart and mind!
JMIBrokers Team
add website/number/social media icons linked to our online pages.";
    $subject='3 Offers Every Trader Dreams to have!';
    Mail::to('ibrahim@jmibrokers.com')->send(new Queued($data,$subject));
    return 'sent';
    ...
    public function login()
    {
        ...
        if (Session::has('user')) {
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/cpanel/home');
            }elseif( Request::segment(1) =='ru'){
                return redirect('ru/cpanel/home');
            }else{
                return redirect('en/cpanel/home');
            }
            ...
            if( Request::segment(1) =='ar'){
                $title = "تسجيل الدخول";
                return view('ar.cpanel.login',compact('title'));
            }elseif( Request::segment(1) =='ru'){
                $title = "Логин клиента";
                return view('ru.cpanel.login',compact('title'));
            }else{
                $title = "Client Login";
                return view('cpanel.login',compact('title'));
            }
    ...
    function checklogin()
     {
        ...
        if ($user) {
            if (Hash::check($input['password'], $user->password)) {
                ...
                if( Request::segment(1) =='ar'){
                    return redirect()->back()->with('Error', 'الأسم المستخدم او كلمة السر غير صحيح');
                }elseif( Request::segment(1) =='ru'){
                    return redirect()->back()->with('Error', 'Имя пользователя или пароль неверны');
                }else{
        ...
        // check username exist or not for js validation

        public function checkusername($username)
        {
            $users=Website_accounts::where('username', $username)->get()->first();
            if(empty($users)){
                return "false";
            }else{
                return "true";
        ...
        // check email exist or not for js validation
        public function checkemail($email)
        {
            $users=Website_accounts::where('email', $email)->get()->first();
            if(empty($users)){
                return "false";
            }else{
                return "true";
...
$client=new Client($accountId, $token);
    ...
    public function signup()
    {
        ...
        if (Session::has('user')) {
            ...
            if( Request::segment(1) =='ar'){
                $title = "حساب جديد ";
                $description="Open an account with JMI Brokers today and trade over 250 instrument";
                $keywords="Signup";
                return view('ar.cpanel.register',compact('title','description','keywords'));
            }elseif( Request::segment(1) =='ru'){
                $title = "Регистрация клиента ";
                $description="Open an account with JMI Brokers today and trade over 250 instrument";
                $keywords="Signup";
                return view('ru.cpanel.register',compact('title','description','keywords'));
            }else{
                $title = "Client Register";
                $description="Open an account with JMI Brokers today and trade over 250 instrument";
                $keywords="Signup";
                return view('cpanel.register',compact('title','description','keywords'));
    ...
    public function post_signup()
    {
       ...
       //         Mail::send([], [], function($message)use( $input){
        //                $message->to('support@jmibrokers.com')->cc('support@jmibrokers.com');
         //               $message->from('info@jmibrokers.com','JMI Brokers');
          //              $message->subject('New Website Account');
           //             $message->setBody('Name : '.$input['fullname'].'<br>'.'UserName : '.$input['username'].'<br>'.'Email : '.$input['email'].'<br>');
             //   });
            ...
            $registered_user=website_accounts::where('email',($input['email']))->get()->first();
                ...
                if( Request::segment(1) =='ar'){
                    $title = "حساب جديد ";
                    return view('ar.cpanel.register',compact('title'));
                }elseif( Request::segment(1) =='ru'){
                    $title = "Регистрация клиента ";
                    return view('ru.cpanel.register',compact('title'));
                }else{
                    $title = "Client Register";
                    return view('cpanel.register',compact('title'));
                }
        ...
        if(fputs($ptr,"WUSERINFO-login=$account->account_id|password=$account->password\nQUIT\n"))
          {
           ...
           while(!feof($ptr))
            ...
            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | رؤية عامة ";
                return view('ar.cpanel.home',compact('title','user','notifications_all','notifications_unseen','accounts','equities','names','balances'));
            }elseif( Request::segment(1) =='ru'){
                $title = "Панель управления | Дом ";
                return view('ru.cpanel.home',compact('title','user','notifications_all','notifications_unseen','accounts','equities','names','balances'));
            }else{
                $title = "Control Panel | Account Overview";
                return view('cpanel.home',compact('title','user','notifications_all','notifications_unseen','accounts','equities','names','balances'));
            }
    ...
    public function change_password()
    {
        ...
        Session::flash('currentPage', 'password');
        $session_user= Session::get('user');
//         if (filter_var($session_user, FILTER_VALIDATE_EMAIL)) {
//
// } else if(!preg_match('/^[A-Za-z][A-Za-z0-9]{5,40}$/', $session_user)){
//
// }else {
//   return redirect()->back()->with('Error', 'An error has been occured');
// }
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',$user->id)->where('notification_status',0)->get()->all();
            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | تغيير كلمة السر ";
                return view('ar.cpanel.change_password',compact('title','user','notifications_all','notifications_unseen'));
            }elseif( Request::segment(1) =='ru'){
                $title = "Панель управления | Change Password ";
                return view('ru.cpanel.change_password',compact('title','user','notifications_all','notifications_unseen'));
            }else{
                $title = "Control Panel | Change Password";
                return view('cpanel.change_password',compact('title','user','notifications_all','notifications_unseen'));
            }
...
if($user->username !== $input['username']){
      ...
      <style>
        *{font-weight: normal ;font-size: 14px;}
        ...
        li span , li span a{color: #4f81bd; font-weight: bold;}
          ...
          .content
     {
              background-image:url("https://www.jmibrokers.com/assets/m/img/new-4.jpg");
 ...
 <body>
  ...
  <div id="logo"> <img src="https://www.jmibrokers.com/assets/m/img/new-5.png"></div>
...
time to time by JMI Brokers LTD, at its sole discretion.</p>
      ...
      <hr>
      <h5 style="font-weight: bold; display: flex;align-items: center;margin:0px;" class=""><img style="margin-right: 5px;    float: left;" src="https://www.jmibrokers.com/assets/m/img/arrow.png"> Contact us</h5>
      ...
      <img src="https://www.jmibrokers.com/assets/m/img/new-3.jpg">
              ...
              $customer_agreement->document='https://www.jmibrokers.com/assets/user_documents/'.$newfilename.'.pdf';
      ...
      if($invoice->save()){
        ...
        $notification1->save();
         ...
         <html>
            ...
            <div class="header">
                <img src="https://www.jmibrokers.com/assets/m/img/new-1.png">
            </div>
 ...
 */
            ...
            $checkout = new Checkout("JMIBrokers LTD",
                'Funding Account Number '.$account_number,
                "fixed_price",
                $amount,
                "USD",
                ['email', 'name']
            ...
            try {
                ...
                $url='https://commerce.coinbase.com/checkout/'.$data['data']['id'];
...
//} catch (\Exception $exception) {
//
//  return redirect()->back()->with('status-success', 'Unable to create checkout. Error: '.$exception->getMessage());
//
//}
          ...
          if( Request::segment(1) =='ar'){
            $url = "https://api.epay.com/paymentApi/merReceive?PAYEE_ACCOUNT=backoffice@jmibrokers.com&PAYEE_NAME=JMI%20BROKERS%20LTD&PAYMENT_AMOUNT=".$amount."&SUGGESTED_MEMO=FUND%20JMI%20BROKERS%20ACCOUNT%20NUMBER%20:%20".$account_number."&INTERFACE_LANGUAGE=EN_US&CHARACTER_ENCODING=UTF-8&PAYMENT_UNITS=USD&STATUS_URL=https://www.jmibrokers.com/ar/epay/success&PAYMENT_URL=https://jmibrokers.com/en&NOPAYMENT_URL=https://www.jmibrokers.com/ar/epay/cancel&V2_HASH=".MD5('backoffice@jmibrokers.com:'.$amount.':USD:b81d40a4c44514fede3aa34a2f71899e')."&";

          	}elseif( Request::segment(1) =='ru'){
              $url = "https://api.epay.com/paymentApi/merReceive?PAYEE_ACCOUNT=backoffice@jmibrokers.com&PAYEE_NAME=JMI%20BROKERS%20LTD&PAYMENT_AMOUNT=".$amount."&SUGGESTED_MEMO=FUND%20JMI%20BROKERS%20ACCOUNT%20NUMBER%20:%20".$account_number."&INTERFACE_LANGUAGE=EN_US&CHARACTER_ENCODING=UTF-8&PAYMENT_UNITS=USD&STATUS_URL=https://www.jmibrokers.com/ru/epay/success&PAYMENT_URL=https://jmibrokers.com/en&NOPAYMENT_URL=https://www.jmibrokers.com/ru/epay/cancel&V2_HASH=".MD5('backoffice@jmibrokers.com:'.$amount.':USD:b81d40a4c44514fede3aa34a2f71899e')."&";

              }else{
                $url = "https://api.epay.com/paymentApi/merReceive?PAYEE_ACCOUNT=backoffice@jmibrokers.com&PAYEE_NAME=JMI%20BROKERS%20LTD&PAYMENT_AMOUNT=".$amount."&SUGGESTED_MEMO=FUND%20JMI%20BROKERS%20ACCOUNT%20NUMBER%20:%20".$account_number."&INTERFACE_LANGUAGE=EN_US&CHARACTER_ENCODING=UTF-8&PAYMENT_UNITS=USD&STATUS_URL=https://www.jmibrokers.com/en/epay/success&PAYMENT_URL=https://jmibrokers.com/en&NOPAYMENT_URL=https://www.jmibrokers.com/en/epay/cancel&V2_HASH=".MD5('backoffice@jmibrokers.com:'.$amount.':USD:b81d40a4c44514fede3aa34a2f71899e')."&";
          ...
          $currency  = $input['currency'];
            ...
            ?>
               ...
               <form name="redirectpost" method="post" action="https://wallet.advcash.com/sci/">
                 ...
                 <!-- Optional Fields -->
                 <input type="hidden" name="ac_success_url" value="https://www.jmibrokers.com/en/cpanel/transaction-history" />
                 <input type="hidden" name="ac_success_url_method" value="GET" />
                 <input type="hidden" name="ac_fail_url" value="https://www.jmibrokers.com/en/cpanel/transaction-history" />
                 <input type="hidden" name="ac_fail_url_method" value="GET" />
                 <input type="hidden" name="ac_status_url" value="https://www.jmibrokers.com/en/cpanel/transaction-history" />
            ...
            array(
              ...
              "ac_success_url"=> 'https://www.jmibrokers.com/advcash/success',
              "ac_success_url_method"=> 'GET',
              "ac_fail_url"=> 'https://www.jmibrokers.com/advcash/fail',
              "ac_fail_url_method"=> 'GET',
              "ac_status_url"=> 'https://www.jmibrokers.com/advcash/status',
...
if($ret1 == Null or $ret1 =='error')
{
    ...
    public function forgotpasswordview()
    {
        ...
        if( Request::segment(1) =='ar'){
            $title="فقدت كلمة السر";
            return view('ar.cpanel.forgot-password',compact('title'));
        }elseif( Request::segment(1) =='ru'){
            $title="Забыл пароль";
            return view('ru.cpanel.forgot-password',compact('title'));
        }else{
        $title="Forgot Password";
            return view('cpanel.forgot-password',compact('title'));
        }
        ...
        //$token3 + token6 + token8 =username of forgot mail
                ...
                $resetlink=URL::to('/en/reset-password/').'/'.$token1.'/'.$token2.'/'.$token3.'/'.$token5.'/'.$token6.'/'.$token7.'/'.$token8;
                Mail::send('mails.forgot-passwordmail',['email' => $email,'resetlink' => $resetlink], function($message)use ($resetlink , $email){
                        ...
                        $message->subject('Reset Your Password');
                            ...
                            if( Request::segment(1) =='ar'){
                                return redirect()->back()->with('status-success', 'لقد تم أرسال الرابط الخاص بتغيير كلمة السر الى بريدك');
                            }elseif( Request::segment(1) =='ru'){
                                return redirect()->back()->with('status-success', 'Мы отправляем ссылку сброса на вашу почту');
                            }else{
                                return redirect()->back()->with('status-success', 'We Sent Reset Link To Your Mail');
                            }
                            ...
                            if( Request::segment(1) =='ar'){
                                return redirect()->back()->with('status-error', 'البريد الألكترونى غير صحيح');
                            }elseif( Request::segment(1) =='ru'){
                                return redirect()->back()->with('status-error', 'Неверный адрес электронной почты или имя пользователя');
                            }else{
                                return redirect()->back()->with('status-error', 'Wrong Mail or Username !!');
                            }
                ...
                if($token1 == md5($user->id) && $token2 == $user->resettoken  && $token5 == md5(md5($user->email)) && $token7 = md5(date('d')) && ($user->resettoken_time+3600) > time()){
                    ...
                    Request::session()->put('userressetpassword', $username);
                            ...
                            if( Request::segment(1) =='ar'){
                                ...
                                return view('ar.cpanel.resetpassword',compact('title'));
                            }elseif( Request::segment(1) =='ru'){
                                $title="Изменить пароль";
                                return view('ru.cpanel.resetpassword',compact('title'));
                            }else{
                                $title="Change Password";
                                return view('cpanel.resetpassword',compact('title'));
                            }
                ...
                $user=website_accounts::where('username', session('userressetpassword'))->get()->first();
                        $this->validate(Request(), [
                            ...
                            if($newpassword == $confirmnewpassword){
                                    ...
                                    if( Request::segment(1) =='ar'){
                                    return Redirect::to('/ar/login')->with('status-success', 'تم تغيير كلمة اسر بنجاح');

                                    }elseif( Request::segment(1) =='ru'){
                                    return Redirect::to('/ru/login')->with('status-success', 'Ваш пароль был успешно изменен');

                                    }else{
                                    return Redirect::to('/en/login')->with('status-success', 'Your Password Has Been Changed Successfully');

                                    }

                            }else{
                                    if( Request::segment(1) =='ar'){
                                        return redirect()->back()->with('status-error', 'كلمة السر يجب ان تكون متطابقة للتأكيد');

                                    }elseif( Request::segment(1) =='ru'){
                                        return redirect()->back()->with('status-error', 'Пароль должен быть равен Подтверждение пароля');

                                    }else{
                                    return redirect()->back()->with('status-error', 'Password Must Equals Confirm Password');
